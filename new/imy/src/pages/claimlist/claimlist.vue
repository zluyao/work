  ...
  <div class="orderlist">
    ...
    <ul class="list-box"
      ...
      <li class="item"
        ...
        <p class="item-order">保单号：{{item.insureBillNum}}</p>
        <p class="item-time">保障期限：{{item.insureBeginDateStr.replace(/-/g,'.')}}-{{item.insureEndDateStr}}</p>
        <div class="apply-btn"
             v-if="(item.claimState === null||item.claimState === 1||item.claimState === 2)&&item.billState===33"
             @click="toClaimOne(item.billId,item.insureSchemeId,item.claimId)">
          <span>申请理赔</span><i class="iconfont icon-chanpintoubaojieshaoye-likegoumai"></i>
          <div class="fiter-shdow"></div>
        </div>
        <img v-if="item.billState==33&&(item.claimState !== null&&item.claimState != 1&&item.claimState != 2)"
             class="is-checking"
             src="../../assets/images/img-claimchecking.png"
             alt="">
      </li>
    </ul>
  </div>
...
export default {
  ...
  methods: {
    toClaimOne(id, insureSchemeId, claimId) {
      localStorage.setItem('insureSchemeId', insureSchemeId);

      window.location.href = `./claimone.html?billId=${id}&claimId=${claimId}`;
    ...
    getList() {
      common.http.get('/api/secret/bill/v1/claimBillList', {
        pageNo: this.pageNo,
